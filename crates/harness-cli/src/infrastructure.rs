...
impl HarnessRepository for SqliteHarnessRepository {
    ...
    fn verify_decision(&self, id: &str) -> Result<DecisionVerifyResult> {
        ...
        let mut command = shell_command(&verify_command);
        let status = command
            .arg(&verify_command)
...
#[cfg(windows)]
fn shell_command(_: &str) -> Command {
...
mod tests {
    ...
    fn decision_verify_runs_from_repo_root() {
        ...
        let pwd_command = if cfg!(windows) {
            format!("cd > {}", pwd_output.display())
        } else {
            format!("pwd > {}", pwd_output.display())
        };
        repository
            .add_decision(DecisionAddInput {
                ...
                doc_path: None,
                verify_command: Some(pwd_command),
